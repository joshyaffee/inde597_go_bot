...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (1, BOARD_DIM, BOARD_DIM)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        self.action_space = list(range(BOARD_DIM ** 2 + 1))  # Fixed list of all possible actions. You should only edit the length
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env = GoEnv.GoBoard(BOARD_DIM)
    ...
    def step(self, action):
        ...
        """
        board, reward, done = self.env.step(self._num_to_coord(action), representation = 1)
        return numpy.array([board]), reward, done
    ...
    def reset(self):
        ...
        """
        self.env.reset()
        return numpy.array([self.env.matrix_board])
    ...
    def action_to_string(self, action_number):
        ...
        """
        return f"{action_number}. {self._num_to_coord(action_number)}"
    
    def _coord_to_num(self, coord):
        if coord.upper() == "PASS":
            return BOARD_DIM ** 2
        else:
            x, y = self.env._coord_to_pos(coord)
            return x * BOARD_DIM + y
        
    def _num_to_coord(self, num):
        if num == BOARD_DIM ** 2:
            return "PASS"
        else:
            x, y = num // BOARD_DIM, num % BOARD_DIM
            return self.env._pos_to_coord(x, y)
